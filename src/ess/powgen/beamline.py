...
def map_detector_to_spectrum(data, *, detector_info):
    if not sc.identical(data.coords['detector'], detector_info.coords['detector']):
        raise sc.CoordError("The 'detector' coords of `data` and `detector_info` do not match.")
    ...
    out.coords['spectrum'] = detector_info.coords['spectrum'] + sc.scalar(1, dtype='int32')
