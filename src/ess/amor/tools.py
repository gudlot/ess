...
def q_grid(q_min=0.008*sc.Unit('1/angstrom'), q_fix=0.005*sc.Unit('1/angstrom'), q_max=0.08*sc.Unit('1/angstrom'), d_q=None):
    """
    ...
    if (q_min < q_fix).value and (q_fix < q_max).value:
        ...
        n_log = (sc.log(q_max / q_fix) / sc.log(1. +
                d_q / q_fix) + 0.5).astype(sc.dtype.int64)
        q_log = sc.Variable(['qz'], values=(q_fix.value * (1. + d_q / q_fix).value **
                            sc.arange('qz', 0, (n_log + 1).values, 1).values), unit=sc.Unit('1/angstrom'))
        ...
        q_linear = q_fix - d_q * sc.arange('qz', n_linear.value, (m_linear - 1).value, -1)
        return q_linear
    elif (q_fix < q_min).value and (q_min < q_max).value:
        n_log = (sc.log(q_max / q_fix) / sc.log(1. +
                 d_q / q_fix) + 0.5).astype(sc.dtype.int64)
        m_log = (sc.log(q_min / q_fix) / sc.log(1. +
                 d_q / q_fix) - 0.5).astype(sc.dtype.int64)
        q_log = sc.Variable(['qz'], values=q_fix.value * (1. + d_q / q_fix).values**sc.arange('qz', m_log.value, (n_log + 1).value, 1.).values, unit=sc.Unit('1/angstrom'))
        return q_log
    else:
        raise ValueError(
