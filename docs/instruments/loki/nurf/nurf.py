...
# possibilites for median filters, I did not benchmark, apparently median_filter could be the faster and medfilt2d is faster than medilt
...
def single_contributions(da):
    ...
    ----------  
    da_dict: dict
            Dictionary that contains spectroscopy data signal (data) from the sample, the reference, and the dark measurement. 
    ...
    # We have already a condition variable 'is_dark' and we separate the incoming sc.DataArray accordingly
    ...
    #GL: Dark and ref have only one spectrum, but data could have multiple in it. So we need to squeeze dark, ref, but not data.
    #SH: It allows for the subsequent operation to broadcast this input, i.e., use the same dark spectrum for all signal spectra.
    #SH: You cannot combine things in operations that have different length along a dimensions.
...
def load_uv(name):
    ...
    ----------  
    uv_dict: dict
        Dictionary that contains UV data signal (data) from the sample, the reference, and the dark measurement. 
        Keys: data, reference, dark
...
def normalize_uv(*, data: sc.DataArray, reference: sc.DataArray, dark: sc.DataArray) -> sc.DataArray:  #<- here the star means that the function accepts only keyword arguments. It eliminates the risk of messing up the order of arguments
    ...
    normalized = sc.log((reference-dark)/(data-dark))  #results in DataArrays with multiple spectra
...
def process_uv(name):
    """ Processses all UV spectra in a single LoKI.nxs and averages them to one corrected UV spectrum.
    ...
    normalized.mean('spectrum'): sc.DataArray
        One averaged UV spectrum. Averaged over all UV spectra contained in the file under UV entry data.
...
def plot_uv(name):
    ...
    out1=sc.plot(sc.collapse(uv_dict['data'], keep='wavelength'),linestyle='dashed', marker='.',grid=True, legend=legend_props, title=f'{name} - raw data', ax=axs[0])
    
    # How to plot dark and reference?
    out4=sc.plot({'dark': uv_dict['dark'], 'reference':uv_dict['reference']},linestyle='dashed', marker='.',grid=True, legend=legend_props, title=f'{name} - dark and reference', ax=axs[1])
    
    # How to plot individual calculated spectra from one .nxs file 
    out2=sc.plot(sc.collapse(normalized, keep='wavelength'),linestyle='dashed', marker='.',grid=True, legend=legend_props, title=f'{name} - calculated', ax=axs[2])

    # How to plot averaged spectra (.mean and NOT .sum)
    out3=sc.plot(sc.collapse(normalized.mean('spectrum'), keep='wavelength'),linestyle='dashed', marker='.',grid=True, legend=legend_props, title=f'{name} - averaged', ax=axs[3])
    #display(figs)


def gather_uv_set(flist_num):
    """ Creates a sc.DataSet for set of given filenames for an experiment composed of multiple, separated UV measurements over time.
    Parameters
    ----------
...
def plot_uv_set(flist_num, lambda_min=None, lambda_max=None, vmin=None, vmax=None):
    ...
    fig_all_spectra=uv_spectra_set.plot(linestyle='dashed', marker='.',grid=True, legend=legend_props, figsize=figure_size)
    if (vmin!=None and vmax!=None):
        fig_all_spectra.ax.set_ylim(vmin,vmax)
    if (lambda_min!=None and lambda_max!=None):
...
def normalize_fluo(*, data: sc.DataArray, reference: sc.DataArray, dark: sc.DataArray) -> sc.DataArray:
...
def plot_fluo(name):
    ...
    out1=sc.plot(sc.collapse(fluo_dict['data']['spectrum',:],keep='wavelength'),linestyle='dashed',grid=True, 
    legend=legend_props, figsize=figure_size, title=f'{name}, raw fluo spectrum - all') #legend={"show": True, "loc": (1.0, 1.0)} #figsize=(width, height)
    display(out1)

    #plot raw and dark spectra for fluo part
    out2=sc.plot({'dark': fluo_dict['dark'], 'reference': fluo_dict['reference']}, linestyle='dashed',grid=True, 
    legend=legend_props, figsize=figure_size, title=f'{name}, dark and reference') #legend={"show": True, "loc": (1.0, 1.0)} #figsize=(width, height))
    display(out2)

    # specific for ILL data, every second sppectrum good, pay attention to range() where the selection takes place
    ...
    out3=sc.plot(only_bad_spectra, linestyle='dashed',grid=True, legend=legend_props, figsize=figure_size, title=f'{name} - all bad raw spectra')
    display(out3)
    ...
    out4=sc.plot(only_good_spectra, linestyle='dashed',grid=True, legend=legend_props, figsize=figure_size, title=f'{name} - all good raw spectra')
    display(out4)
    ...
    out5=sc.plot(only_good_fspectra, linestyle='dashed',grid=True, legend=legend_props, figsize=figure_size, title=f'{name} - all good final spectra')
    display(out5)


    # zoom in final spectra selection
    out6=sc.plot(only_good_fspectra, linestyle='dashed',grid=True, legend=legend_props, figsize=figure_size, title=f'{name} - all good final spectra')
    lambda_min=250
    lambda_max=600
...
def export_uv(name, path_output):
    ...
    normalized=normalize_uv(**uv_dict)  #results in one DataArray, all uv spectra normalized  
    ...
    l=''.join(['uv_spectra_{0}\t'.format(i) for i, x in enumerate(range(normalized.sizes['spectrum']))])
    hdrtxt='wavelength [nm]\t uv_spectra_avg\t'
    final_header=hdrtxt+l
 
    data_to_save= np.column_stack((normalized.coords['wavelength'].values.transpose(), normalized_avg.data.values.transpose(), normalized.data['spectrum',:].values.transpose()))
...
def export_fluo(name, path_output):
    ...
    data_to_save= np.column_stack((final_fluo.coords['wavelength'].values.transpose(), final_fluo.data['spectrum',:].values.transpose()))

    # dump the content
    with open(path_to_save, 'w') as f:
        np.savetxt(f, data_to_save, fmt=''.join(['%.5f\t']+['%.5e\t']*final_fluo.sizes['spectrum']), delimiter='\t', header=final_header)




def fluo_maxint_max_wavelen(flist_num,wllim=300, wulim=400, wl_unit=None, medfilter=True, kernel_size=15):
    ...
    for name in flist_num:
        ...
        for mwl in unique_monowavelen:
            fluo_int_max=fluo_filt_max['intensity_max'][fluo_filt_max.coords['monowavelengths']==mwl*sc.Unit('nm')].values  #returns a numpy array
            fluo_wavelen_max=fluo_filt_max['wavelength_max'][fluo_filt_max.coords['monowavelengths']==mwl*sc.Unit('nm')].values  #returns a numpy array

            # I collect the values in a dict with nested dicts, separated by wavelength 
            fluo_int_dict[f"{mwl}{sc.Unit('nm')}"][f'{name}']={'intensity_max': fluo_int_max,'wavelength_max': fluo_wavelen_max }
...
def fluo_plot_maxint_max_wavelen(fluo_int_dict: dict):
    ...
    for j,mwl in enumerate(fluo_int_dict):
        ...
        for i, fname in enumerate(max_int_dict):
            ...
            ax[j,1].plot(x_val,max_int_wavelen_dict[fname], marker='o', linestyle = 'None')
        ...
        ax[j,1].plot(np.linspace(0, len(max_int_wavelen_dict.keys())-1, num=len(max_int_wavelen_dict.keys())),[int(mwl[:-4])]*len(max_int_wavelen_dict.keys()), '--')
        #overwrite xticks in ax0
        ax[j,0].set_xticks(np.arange(0,len(max_int_dict.keys()),1), labels=[f'{name}' for name in max_int_dict], rotation=90)
        #overwrite xticks in ax1
        ax[j,1].set_xticks(np.arange(0,len(max_int_wavelen_dict.keys()),1), labels=[f'{name}' for name in max_int_wavelen_dict], rotation=90)
    

def apply_medfilter(da: sc.DataArray, kernel_size: Optional[int] = None) -> sc.DataArray:    #https://docs.python.org/3/library/typing.html#typing.Optional
    ...
    out1=sc.plot(sc.collapse(da, keep='wavelength'),linestyle='dashed', marker='.',grid=True, legend=legend_props, title=f'Before median filter', ax=axs[0])
    out2=sc.plot(sc.collapse(da_mfilt, keep='wavelength'),linestyle='dashed', marker='.',grid=True, legend=legend_props, title=f'After median filter - kernel_size: {kernel_size}', ax=axs[1])
    #display(figs)
...
def uv_peak_int(uv_da: sc.DataArray, wavelength=None, wl_unit=None, tol=None):
    ...
    """
    assert 'wavelength' in uv_da.dims, 'sc.DataArray is missing the wavelength dimension'  #assert that 'wavelength' is a dimension in the uv_da sc.DataArray
    ...
    if wl_unit is None:
        ...
        if not isinstance(wl_unit, sc.Unit):
            raise TypeError
        assert wl_unit == uv_da.coords['wavelength'].unit  #we check that the given unit corresponds to the unit for the wavelength
    ...
    uv_da_filt=uv_da['wavelength', (wavelength-tol)*wl_unit: (wavelength+tol)*wl_unit]
    ...
    x2 = sc.linspace(dim='wavelength', start=wavelength, stop= wavelength, num=1, unit=wl_unit)
    # alternative
    ...
    uv_peak_int={'one_wavelength':uv_int_one_wl, 'wl_interval':uv_int_mean_interval, 'wavelength': wavelength, 'unit': wl_unit, 'tol': tol}

    return uv_peak_int

def plot_uv_peak_int(uv_da: sc.DataArray, name, wavelength=None, wl_unit=None, tol=None, medfilter=False, kernel_size=None):
    ...
    #process 
    uv_peak_int_dict=uv_peak_int(uv_da, wavelength=wavelength, wl_unit=wl_unit, tol=tol)
    #append name to dict
    ...
    out1=sc.plot(uv_peak_int_dict['one_wavelength']['spectrum',:].squeeze(), linestyle='none',grid=True, title=f"UV peak intensity for {uv_peak_int_dict['wavelength']} {uv_peak_int_dict['unit']} in {name} ",  ax=ax0) 
    ax0.set_ylabel('UV int')
    ax1 = fig.add_subplot(gs[0, 1])
    out2=sc.plot(uv_peak_int_dict['wl_interval']['spectrum',:], linestyle='none',grid=True,
         title=f"UV peak intensity for intervall [{uv_peak_int_dict['wavelength']-uv_peak_int_dict['tol']} , {uv_peak_int_dict['wavelength']+uv_peak_int_dict['tol']}]{uv_peak_int_dict['unit']} in {name} ",  ax=ax1) 
    ax1.set_ylabel('UV int')

def uv_quick_data_check(filelist, wavelength=None, wl_unit=None, tol=None, medfilter=False, kernel_size=None):
    """ Plots uv peak intensity for a given wavelength with unit wl_unit for a given filelist in order of items in filelist
        Per recorded spectrum in the file a data point is shown.
    ...
    if medfilter is not False:
        ...
        uv_dict={name:apply_medfilter(uv_dict[name], kernel_size=kernel_size) for name in filelist}
    ...
    uv_peak_int_dict={name:uv_peak_int(uv_dict[name], wavelength=wavelength, wl_unit=wl_unit, tol=tol) for name in filelist}

    # prepare plots
    figure_size=(20,10)
    fig=plt.figure(figsize=figure_size)
    gs = gridspec.GridSpec(nrows=2, ncols=3, width_ratios=[1, 1, 1], wspace=0.3, height_ratios=[1,1])
    ax0= fig.add_subplot(gs[0, 0])
    ax1= fig.add_subplot(gs[0, 1])
    ...
    for name in filelist:
        ...
        ax0.plot(uv_peak_int_dict[f'{name}']['one_wavelength']['spectrum',:].values, 'o', label=f'{name}')
        ax0.legend()
        
        ax1.plot(uv_peak_int_dict[f'{name}']['wl_interval']['spectrum',:].values, 'o', label=f'{name}')
        ax1.legend()
        

    ax0.grid(visible=True)
    ax0.set_title(f"UV peak intensity for {uv_peak_int_dict[filelist[0]]['wavelength']} {uv_peak_int_dict[filelist[0]]['unit']} ")
    ax0.set_xlabel('spectrum')

    ax1.plot(uv_peak_int_dict[filelist[0]]['wl_interval']['spectrum',:].values, 'o', label=f'{name}')
    ax1.legend()
    ax1.grid(visible=True)
    ax1.set_title(f"UV peak intensity for intervall [{uv_peak_int_dict[filelist[0]]['wavelength']-uv_peak_int_dict[filelist[0]]['tol']} , {uv_peak_int_dict[filelist[0]]['wavelength']+uv_peak_int_dict[filelist[0]]['tol']}]{uv_peak_int_dict[filelist[0]]['unit']}")
    print(uv_peak_int_dict[filelist[0]]['wl_interval']['spectrum',:].values, uv_peak_int_dict[filelist[0]]['wl_interval']['spectrum',:].values)
    ax1.set_xlabel('spectrum')

    display(fig)    

def plot_multiple_uv_peak_int(filelist, wavelength=None, wl_unit=None, tol=None, medfilter=False, kernel_size=None):
    """ Plots uv peak intensity for a given wavelength with unit wl_unit for a given filelist in order of items in filelist
    ...
    gs = gridspec.GridSpec(nrows=1, ncols=2, width_ratios=[1, 1], wspace=0.3, height_ratios=[1])
    ax0= fig.add_subplot(gs[0, 0])
    ax1= fig.add_subplot(gs[0, 1])
    ...
    for i, name in enumerate(filelist):
        ...
        ax0.plot(i*np.ones(num_spec), uv_peak_int_dict['one_wavelength']['spectrum',:].values, 'o', label=f'{name}')
        ax0.legend()
        ax0.grid(visible=True)
        ax0.set_title(f"UV peak intensity for {uv_peak_int_dict['wavelength']} {uv_peak_int_dict['unit']} ")

        
        ax1.plot(i*np.ones(num_spec), uv_peak_int_dict['wl_interval']['spectrum',:].values, 'o', label=f'{name}')
        ax1.legend()
        ax1.grid(visible=True)
        ax1.set_title(f"UV peak intensity for intervall [{uv_peak_int_dict['wavelength']-uv_peak_int_dict['tol']} , {uv_peak_int_dict['wavelength']+uv_peak_int_dict['tol']}]{uv_peak_int_dict['unit']}")

    #overwrite xticks in ax0
    ax0.set_xticks(np.arange(0,len(filelist),1), labels=[f'{name}' for name in filelist], rotation=90)
    #overwrite xticks in ax1
    ax1.set_xticks(np.arange(0,len(filelist),1), labels=[f'{name}' for name in filelist], rotation=90)
...
def uv_turbidity_fit(uv_da: sc.DataArray, wl_unit=None, fit_llim=None, fit_ulim=None, b_llim=None, b_ulim=None, m=None, plot_corrections=True):
    ...
    b0=uv_da['wavelength', b_llim*wl_unit: b_ulim*wl_unit].mean(dim='wavelength').values
    ...
    for i in range(num_spec):
        # start parameters
        p0 = [b0[i], m0[i]]
        popt, pcov = leastsq(residual, p0, args=(uv_da_filt.coords['wavelength'].values, uv_da_filt['spectrum',i].values))
                
        # calculation of each spectrum corrected for fitted turbidity
        uv_da_turb_corr_dat[i,:]=uv_da['spectrum',i].values- turbidity(uv_da['spectrum',i].coords['wavelength'].values,popt[0], popt[1])
    ...
    uv_da_turbcorr.attrs['fit-slope_m'] = sc.array(dims=['spectrum'], values=res_popt[:,1])
    uv_da_turbcorr.attrs['fit-offset_b'] = sc.array(dims=['spectrum'], values=res_popt[:,0])
    uv_da_turbcorr.attrs['fit-pcov'] = sc.array(dims=['spectrum'], values=res_pcov[:,0])
    ...
    if plot_corrections:
        ...
        out0=sc.plot(sc.collapse(uv_da, keep='wavelength'), grid=True, title='before correction', ax=ax2[0])
        out1=sc.plot(sc.collapse(uv_da_turbcorr, keep='wavelength'), grid=True, title='after correction', ax=ax2[1])

        fig, ax=plt.subplots(ncols=num_spec+1, figsize=(18, 5))
        out3=sc.plot(sc.collapse(uv_da_filt,keep='wavelength'), grid=True, title='Selection for turbidity', ax=ax[-1])

        for i in range(num_spec):
            # collect the fitting parameters for each spectrum to avoid new fitting
            popt=[uv_da_turbcorr.attrs['fit-offset_b']['spectrum',i].values, uv_da_turbcorr.attrs['fit-slope_m']['spectrum',i].values]
          
            ax[i].plot(uv_da.coords['wavelength'].values, uv_da['spectrum',i].values, 's', label=f'Full UV raw data {i}')
            ax[i].plot(uv_da.coords['wavelength'].values, uv_da['spectrum',i].values - turbidity(uv_da.coords['wavelength'].values,popt[0], popt[1] ), 'x', label=f'Whole UV spectrum, fitted turbidity subtracted {i}')
            ax[i].plot(uv_da.coords['wavelength'].values,turbidity(uv_da.coords['wavelength'].values,popt[0], popt[1] ) , '^', label=f'Full turbidity {i}' )
            
            ax[i].plot(uv_da_filt.coords['wavelength'].values,turbidity(uv_da_filt.coords['wavelength'].values,popt[0], popt[1] ) , '.', label=f'Fitted turbidity {i}, b={popt[0]:.3f}, m={popt[1]:.3f}' )
            ...
            ax[i].plot(uv_da['wavelength', b_llim*wl_unit: b_ulim*wl_unit].coords['wavelength'].values, uv_da['wavelength', 
            b_llim*wl_unit: b_ulim*wl_unit]['spectrum',i].values,'v', label=f'Selection for b0 {i}')         
            ...
            ax[i].set_ylim([-0.5, 1.1*uv_da['spectrum',i].values[np.isfinite(uv_da['spectrum',i].values)].max()])
...
def uv_multi_turbidity_fit(filelist, wl_unit=sc.Unit('nm'), fit_llim=300, fit_ulim=850, b_llim=450, b_ulim=700,m=0.1, plot_corrections=False):
    ...
    for name in filelist:
        uv_dict=load_uv(name)
        uv_da=normalize_uv(**uv_dict)
        uv_da_turbcorr=uv_turbidity_fit(uv_da,  wl_unit=wl_unit, fit_llim=fit_llim, fit_ulim= fit_ulim, b_llim=b_llim, b_ulim=b_ulim ,m=m, plot_corrections=plot_corrections )
        # append names as attributes 
        uv_da_turbcorr.attrs['name']=sc.array(dims=['spectrum'], values=[name]*uv_da_turbcorr.sizes['spectrum'])
        uv_collection[f'{name}']=uv_da_turbcorr

        #print(name,uv_da_turbcorr.data.shape,uv_da_turbcorr.data.values.ndim  )

    multi_uv_turb_corr_da=sc.concat([uv_collection[f'{name}'] for name in filelist], dim='spectrum')
    ...
    out=sc.plot(sc.collapse(multi_uv_turb_corr_da, keep='wavelength'),grid=True, ax=ax[0], legend=legend_props, title=f'All turbidity corrected UV spectra for {num_spectra} spectra')
    ax[0].set_ylim([-1, 1.2*multi_uv_turb_corr_da.data.values[np.isfinite(multi_uv_turb_corr_da.data.values)].max()])

    out2=sc.plot(sc.collapse(multi_uv_turb_corr_da.attrs['fit-offset_b'],keep='spectrum'), title=f'All fit-offset b for {num_spectra} spectra', ax=ax[1], grid=True)
    
    
    #ax0.set_xticks(np.arange(0,len(filelist),1), labels=[f'{name}' for name in filelist], rotation=90)
    secx=ax[1].secondary_xaxis(-0.2)
    secx.set_xticks(np.arange(0,num_spectra,1), labels=[f'{name}' for name in multi_uv_turb_corr_da.attrs['name'].values], rotation=90)
    out3=sc.plot(sc.collapse(multi_uv_turb_corr_da.attrs['fit-slope_m'],keep='spectrum'), title=f'All fit-slope m for {num_spectra} spectra', ax=ax[2], grid=True)
    secx2=ax[2].secondary_xaxis(-0.2)
    secx2.set_xticks(np.arange(0,num_spectra,1), labels=[f'{name}' for name in multi_uv_turb_corr_da.attrs['name'].values], rotation=90)
...
def fluo_peak_int(fluo_da: sc.DataArray, wllim=None, wulim=None, wl_unit=None, medfilter=True, kernel_size=None):
    ...
    if wl_unit is None:
        ...
        if not isinstance(wl_unit, sc.Unit):
            raise TypeError
        assert wl_unit == fluo_da.coords['wavelength'].unit  #we check that the given unit corresponds to the unit for the wavelength
    ...
    assert wllim<wulim, "Lower wavelength limit needs to be smaller than upper wavelength limit"
    ...
    #new sc.Dataset
    fluo_filt_max=sc.Dataset()
    fluo_filt_max['intensity_max']=sc.Variable(dims=['spectrum'], values=fluo_filt_max_int)
    fluo_filt_max['wavelength_max']=sc.Variable(dims=['spectrum'], values= fluo_filt_max_wl, unit=wl_unit)
...
def plot_fluo_peak_int(fluo_da: sc.DataArray, name, wllim=None, wulim=None, wl_unit=None, medfilter=True, kernel_size=None):
    ...
    out0=sc.plot(fluo_filt_max['intensity_max']['spectrum',:],grid=True,labels={'spectrum': 'monowavelengths'}, title=f'Sample: {name}', ax=ax0)
    out1=sc.plot(fluo_filt_max['wavelength_max']['spectrum',:],grid=True, labels={'spectrum': 'monowavelengths'}, title=f'Sample: {name}', ax=ax1)
    display(fig)
