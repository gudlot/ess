...
def test_merge_calibration_add_all_parameters(calibration):
    da = sc.DataArray(sc.ones(sizes=calibration.sizes),
                      coords={
                          'spectrum':
                          sc.arange('spectrum',
                                    calibration.sizes['spectrum'],
                                    unit=None)
                      })
...
def test_merge_calibration_raises_if_spectrum_mismatch(calibration):
    da = sc.DataArray(sc.ones(sizes=calibration.sizes),
                      coords={
                          'spectrum':
                          sc.zeros(sizes={'spectrum': calibration.sizes['spectrum']},
                                   unit=None)
                      })
    ...
    da = sc.DataArray(sc.ones(sizes=calibration.sizes),
                      coords={
                          ...
                          sc.arange('spectrum',
                                    ...
                                    unit=None),
                          'difa':
                          sc.ones(sizes={'spectrum': calibration.sizes['spectrum']})
                      })
    with pytest.raises(ValueError):
        merge_calibration(into=da, calibration=calibration)
